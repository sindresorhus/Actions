...
struct GetMapImageOfLocation: AppIntent {
	...
	// iOS & visionOS do not support generating the image in the background.
	#if canImport(UIKit)
	...
	func perform() async throws -> some IntentResult & ReturnsValue<IntentFile> {
		...
		let snapshot = try await withTaskCancellationHandler {
			try await snapshotter.start(with: .global())
		} onCancel: {
			snapshotter.cancel()
		}
		...
		let result = try image.toIntentFile(filename: UUID().uuidString) // We use random filename to work around Shortcuts issue when it's run multiple times. (iOS 17.5)
