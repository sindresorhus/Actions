		...
		VStack(spacing: 16) {
			...
			VStack {
				Text("This app has no user-interface. It provides a bunch of useful actions that you can use when creating shortcuts.")
					.multilineText()
					.padding()
				Text("You have to restart your device for the actions to show up in the Shortcuts app. If you ever get an error (like “errorDomain error 2”) when running the actions, restart your device.")
					.font(.system(SSApp.isFirstLaunch ? .title2 : .body))
					.bold()
					.padding(.bottom)
				Text(OS.isMacOS ? "Open the Shortcuts app, click the “Apps” tab in the right sidebar, and then click the “Actions” app." : "Open the Shortcuts app, tap the “Apps” tab in the action picker, and then tap the “Actions” app.")
					.multilineText()
					.secondaryTextStyle()
					.padding(.horizontal)
			}
